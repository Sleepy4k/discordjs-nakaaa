...
module.exports.registerPlayerEvents = (player) => {
  ...
  player.on("connectionError", async (queue, error) => {
    ...
    if (error.code === "ABORT_ERR") {
      await queue.metadata
        .send("❌ | Could not join your voice channel!")
        .catch(() => {});
    } else {
      await queue.metadata.send("❌ | Something went wrong!").catch(() => {});
    }
  ...
  player.on("tracksAdd", async (queue, tracks) => {
    ...
    let totalTime = `${hour < 1 ? " " : ` ${hour} hr`}${
      minutes < 1 ? " " : ` ${minutes} min`
    }${seconds < 10 ? " " : ` ${seconds} sec`}`;

    await queue.metadata
      .send(
        `🎶 | Added **${tracks.length}** tracks to the queue. **${queue.tracks.length} songs,${totalTime}**`
      )
      .catch(() => {});
  });
